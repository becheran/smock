...
func GenerateMock(res model.InterfaceResult, version string) (mock []byte, err error) {
	...
	res.Imports = addMissingImports(res.Imports)
...
func addMissingImports(imports []model.Import) []model.Import {
	...
	if !fmtAlreadyImported {
		imports = append(imports, model.Import{Path: "fmt"})
	}
	if !reflectAlreadyImported {
		imports = append(imports, model.Import{Path: "reflect"})
	}
	if !syncAlreadyImported {
		imports = append(imports, model.Import{Path: "sync"})
	}
	if !runtimeAlreadyImported {
		imports = append(imports, model.Import{Path: "runtime"})
	}

	sort.SliceStable(imports, func(a, b int) bool {
		return strings.Compare(imports[a].ImportName(), imports[b].ImportName()) < 0
	})

	return imports
}

func printHelperInterface(w *writer) {
	w.P("t interface {")
	w.Ident()
