...
func GenerateMock(res model.InterfaceResult) (mock string, err error) {
	...
	testInterfaceName := fmt.Sprintf("%s%sTest", MockPrefix, res.Name)
	mockedStructName := fmt.Sprintf("%s%s", MockPrefix, res.Name)
	...
	w.P("package %s%s", res.PackageName, PackageSuffix)
	w.P("")
	...
	w.P("func New%s(t MockHandlerTest) *%s {", mockedStructName, mockedStructName)
	w.Ident()
